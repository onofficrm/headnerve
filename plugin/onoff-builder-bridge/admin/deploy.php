...
if ($license_ok && function_exists('icrm_builder_deploy_check_status')) {
    ...
    $deploy_message = 'iCRM 라이선스를 먼저 설정하세요.';
} else {
    $deploy_message = '빌더 배포 모듈이 없습니다. iCRM 기능 업데이트(2026.06.08.2+)를 적용하세요.';
}
...
<header class="onoff-builder-admin__header">
  <div class="onoff-builder-admin__inner">
    <p class="onoff-builder-admin__brand">
      ...
      <span class="onoff-builder-admin__brand-sub">iCRM 중앙 배포</span>
    </p>
...
<main class="onoff-builder-admin__main">
  <div class="onoff-builder-admin__inner">
    <div class="onoff-builder-admin__page-head">
      <h1>그누보드에 배포</h1>
      <p class="onoff-builder-admin__lead">프로젝트 <strong><?php echo onoff_builder_escape($project_name); ?></strong> (<code><?php echo onoff_builder_escape($project_id); ?></code>)를 iCRM에 등록합니다. 사이트 관리자가 <strong>iCRM 업데이트 → 빌더 디자인 적용</strong>으로 반영합니다.</p>
    </div>

    <div class="onoff-builder-admin__help">
      <p><strong>배포 흐름</strong></p>
      <ol style="margin:0;padding-left:1.25rem">
        <li>아래 [iCRM에 배포 등록] — 중앙 서버에 릴리스 생성</li>
        <li>사이트 관리자 → <a href="<?php echo onoff_builder_escape($apply_admin_url); ?>">iCRM 업데이트</a> → 빌더 디자인 적용</li>
    ...
    <div class="onoff-builder-admin__form" style="margin-top:1rem">
      ...
      <form method="post" action="<?php echo onoff_builder_escape(onoff_builder_admin_url('deploy_update.php')); ?>" class="onoff-builder-admin__form-actions">
        ...
        <button type="submit" class="onoff-builder-admin__btn onoff-builder-admin__btn--primary" <?php echo ($license_ok && function_exists('icrm_builder_deploy_publish_project')) ? '' : 'disabled'; ?>>iCRM에 배포 등록</button>
        ...
        <a class="onoff-builder-admin__btn" href="<?php echo onoff_builder_escape($apply_admin_url); ?>">iCRM 업데이트</a>
        <?php } ?>
      </form>
    </div>
  </div>
