...
function icrm_install_load_saved_config()
{
    ...
    if (!is_file($path)) {
        return array('license_key' => '', 'api_base_url' => '');
    }
    $license = '';
    $api = '';
    if (defined('ICRM_UPDATE_LICENSE_KEY')) {
        $license = trim((string) ICRM_UPDATE_LICENSE_KEY);
    }
    if (defined('ICRM_UPDATE_API_BASE_URL')) {
        $api = trim((string) ICRM_UPDATE_API_BASE_URL);
    }
...
function icrm_install_save_config($license_key, $api_base_url)
{
    ...
    $php = "<?php\nif (!defined('_GNUBOARD_')) exit;\n"
        . "define('ICRM_UPDATE_LICENSE_KEY', " . var_export($license_key, true) . ");\n"
        . "define('ICRM_UPDATE_API_BASE_URL', " . var_export($api_base_url, true) . ");\n";

    return file_put_contents(icrm_install_config_path(), $php, LOCK_EX) !== false;
}
...
{
    if ($license === '') {
        return array('success' => false, 'message' => 'iCRM 라이선스 키를 입력하세요. (온오프마케팅 iCRM에서 발급)');
    }
    ...
    if (empty($manifest['success']) || empty($manifest['release_id'])) {
        return array(
            'success' => false,
            'message' => isset($manifest['message']) ? $manifest['message'] : 'iCRM manifest 조회 실패',
        );
    }
...
<body>
<div class="box">
    <h1>iCRM 업데이트 1회 설치</h1>
    <p class="sub">구버전 사이트에 업데이트 기능을 처음 설치합니다. SEO 메타 메뉴가 없어도 됩니다.</p>

    <?php if ($done && $step !== 'run') { ?>
        <div class="ok">이미 설치되어 있습니다.</div>
        <p><a class="btn" href="<?php echo icrm_install_h(G5_PLUGIN_URL . '/icrm_update/admin/index.php'); ?>">iCRM 업데이트 화면</a></p>
    <?php } elseif ($step === 'run' && is_array($result)) { ?>
        <?php if (!empty($result['success'])) { ?>
            ...
            <p>관리자 → <strong>환경설정 → iCRM 업데이트</strong> 메뉴가 생겼습니다.</p>
            <a class="btn" href="<?php echo icrm_install_h($result['admin_url']); ?>">iCRM 업데이트 화면</a>
    ...
    <?php } else { ?>
        <div class="info">
            ...
            • iCRM 서버에 g5-update API·릴리스 파일이 <strong>실제 배포</strong>되어 있어야 합니다.<br>
            • 라이선스 키는 온오프마케팅 iCRM에서 해당 도메인(<code><?php echo icrm_install_h(icrm_install_domain()); ?></code>)용으로 발급받으세요.
        </div>
        ...
        <form method="post" action="?step=run">
            <label for="license_key">iCRM 라이선스 키 *</label>
            <input type="password" name="license_key" id="license_key" value="<?php echo icrm_install_h($form_license); ?>" placeholder="온오프마케팅 iCRM에서 발급" autocomplete="off">

            <label for="api_base_url">iCRM g5-update API URL</label>
