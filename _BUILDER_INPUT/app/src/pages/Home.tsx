...
export function Home() {
  ...
  return (
    <div className="w-full h-full relative" style={{ height: '100dvh', paddingTop: 0 }}>
      <div 
        ref={containerRef}
        className="maekrak-snap-container bg-white"
      >
        <HeroSection />
        <PhilosophySection />
        <DepartmentsSection />
      ...
      {/* Floating Dots Navigation - Amondps style */}
      <div className="hidden lg:flex fixed right-10 top-1/2 -translate-y-1/2 flex-col gap-6 z-40 items-center">
        {sections.map((sec, idx) => (
          ...
          >
            <div className={`absolute w-full h-full rounded-full transition-all duration-300 ${activeSection === idx ? 'bg-maekrak-accent scale-[2.5]' : 'bg-gray-300 group-hover:bg-gray-500'}`} />
            {activeSection === idx && (
