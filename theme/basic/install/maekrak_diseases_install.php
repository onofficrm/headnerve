<?php
/**
 * 맥락한의원 2층 질환 상세 content DB 일괄 등록 (4차, 1회 실행)
 * /theme/basic/install/maekrak_diseases_install.php?key=mrk_dis_4th_20260520
 * 실행 후 서버에서 이 파일을 삭제하세요.
 */
define('MAEKRAK_DISEASE_INSTALL_KEY', 'mrk_dis_4th_20260520');
...
if (!$allowed && MAEKRAK_DISEASE_INSTALL_KEY !== '' && isset($_GET['key']) && $_GET['key'] === MAEKRAK_DISEASE_INSTALL_KEY) {
    $allowed = true;
}
...
echo '<h1>맥락한의원 2층 질환 상세 등록 (' . count($pages) . '건)</h1><ul>';
foreach ($done as $line) {
    echo '<li>' . htmlspecialchars($line) . '</li>';
}
echo '</ul><p>스킨: <strong>' . htmlspecialchars($skin) . '</strong></p>';
echo '<p><strong>보안: 이 install 파일을 서버에서 삭제하세요.</strong></p>';
echo '<p><a href="' . G5_URL . '">홈으로</a></p>';
